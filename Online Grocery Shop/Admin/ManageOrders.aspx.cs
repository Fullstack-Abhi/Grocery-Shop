...
public partial class Admin_ManageOrders : System.Web.UI.Page
{
    ...
    protected void btnExport_Click(object sender, EventArgs e)
    {
        ...
        {
            using (SqlCommand cmd = new SqlCommand("Select RTRIM(Order_Code) as [Order Code],Order_Time as [Order Date],RTRIM(Productname) as [Product Name],RTRIM(OrderedProducts.Price) as [Price (Rs)],RTRIM(Qty) as [Qty],RTRIM(Unit) as [Unit],RTRIM(Name) as [Customer name],RTRIM(Address + ',' + City + ',' + State + ',' + PostalCode) as [Address],RTRIM(TotalAmount) as [Total Amount (Rs)] from [Order],OrderedProducts,Product where product.ProductID=Orderedproducts.Prod_ID and [Order].Order_Code=OrderedProducts.OrderCode and Status='Confirmed' order by Order_Time"))
            {
